...
- `LyricsProvider` accepts only plausible title/artist/duration matches and caches raw LRC under the user cache directory. Manual reload bypasses cache; mismatched-duration cache entries are rejected.
...
- `AppDelegate` receives event-driven seek updates through the bundled MediaRemoteAdapter, performs a full reconciliation every 15 seconds, and schedules a zero-tolerance one-shot timer for the next lyric boundary.
